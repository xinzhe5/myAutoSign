...
- 当前版本：`2.2.15`
- `2.2.15`（2026-06-06）：
  - 账号管理页面恢复四个操作图标在中间操作区显示
  - 账号状态标签移动到操作图标同一行，位于账号信息和余额信息之间
- `2.2.14`（2026-06-06）：
  - 账号管理页面账号状态标签和签到、打开、编辑、删除图标合并到同一行显示
- `2.2.13`（2026-06-06）：
  - 账号管理页面账号信息恢复左对齐，卡片宽度恢复原布局
  - 账号名称、用户名、状态标签、操作图标和余额信息在卡片中上下居中对齐
- `2.2.12`（2026-06-06）：
  - 账号管理页面账号卡片整体限宽并居中显示
  - 账号名称、用户名和状态标签行改为居中对齐
- `2.2.11`（2026-06-06）：
  - 账号管理页面账号卡片的签到、打开、编辑、删除图标固定一行显示
- `2.2.10`（2026-06-06）：
  - 账号管理页面账号卡片不再显示站点地址、用户 ID 和最近时间
  - 账号卡片操作改为紧凑图标按钮，并放在账号信息和余额信息中间
  - 账号管理页面新增余额、今日消费和今日收入显示，布局与侧边栏保持一致
- `2.2.9`（2026-06-06）：
  - 添加或编辑书签时，标签输入框下方会显示已有标签，点击即可选择或取消选择
  - 标签输入框仍可直接输入新标签，保存后会成为可复用标签
  - 侧边栏“账号信息”区域新增“新增书签”入口，可快速把当前标签页保存为书签
- `2.2.8`（2026-06-06）：
  - 侧边栏选择“自动添加”后不再立即获取账号信息，改为进入表单后点击“自动识别当前站点”才开始识别
  - 自动添加表单会先预填当前站点地址，并提示下一步识别操作
- `2.2.7`（2026-06-06）：
...
加载完成后，点击扩展图标会打开侧边栏。侧边栏会显示当前站点和已保存账号信息，点击“新增账号”后可选择自动添加或手动添加，也可以点击“新增书签”快速保存当前标签页。还可以在扩展列表里打开“扩展程序选项”，或在侧边栏中点击“设置”进入完整设置页。

完整设置页分为六个视图：

- `账号管理`：添加、编辑、删除账号，支持站点类型下拉选择和自动识别。
- `模型列表`：按账号刷新模型目录，支持搜索、提供商筛选、排序和复制模型名。
- `书签管理`：添加、编辑、删除、置顶、搜索和打开常用入口；添加或编辑时可选择已有标签，也可直接输入新标签。
